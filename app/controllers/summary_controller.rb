...
class SummaryController < ApplicationController
  ...
  private
    ...
    def summary_for_period(range, period, type)
      ...
      { :period => period,
        :type => type,
        :duration => Time.at(PowerCalculator::total(workouts.collect {|workout| workout.marker.duration.to_i}) || 0).utc,
        :distance => PowerCalculator::total(workouts.collect {|workout| workout.marker.distance}) || 0.0,
        :energy => PowerCalculator::total(workouts.collect {|workout| workout.marker.energy}) || 0 }
    end
